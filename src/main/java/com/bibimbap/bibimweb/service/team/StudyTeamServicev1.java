...
public class StudyTeamServicev1 {
    ...
    public StudyDetailResponseDto getStudyDetailById(Long detailId) {
        return makeDetailResponseDto(teamPostRepository.findById(detailId).get());
    }
    ...
    public StudyDetailResponseDto addStudyDetail(StudyDetailCreateDto dto) {
        ...
        TeamPost saved = teamPostRepository.save(TeamPost.builder()
                .team(studyTeam)
                .week(dto.getWeek())
                .content(dto.getContent())
                .build());
        studyTeam.getPosts().add(saved);
    ...
    public StudyDetailResponseDto updateStudyDetail(StudyDetailUpdateDto dto) {
        TeamPost detail = teamPostRepository.findById(dto.getId()).get();
        detail.setWeek(dto.getWeek());
        detail.setContent(dto.getContent());

        dto.getAttendances().stream()
                .forEach(attendance -> {
                    StudyRole studyRole = studyRoleRepository.findByTeamIdAndRollNameAndMemberId(detail.getTeam().getId(), RoleName.MEMBER.name(), attendance.getMemberId()).get();
                    List<Boolean> attendanceList = toAttendanceList(studyRole.getAttendance());
        ...
        TeamPost saved = teamPostRepository.save(detail);
        return makeDetailResponseDto(saved);
    }
    ...
    private StudyDetailResponseDto makeDetailResponseDto(TeamPost detail) {
        StudyDetailResponseDto res = mapper.map(detail, StudyDetailResponseDto.class);
        List<AttendanceResponseDto> attendances = detail.getTeam().getMemberRoles().stream()
                .filter(mr -> mr.getRollName().equals(RoleName.MEMBER.name()))
                .map(mr -> {
                    StudyRole studyRole = (StudyRole) mr;
